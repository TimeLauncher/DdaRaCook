...
> 아래 소유·경계는 **금지가 아니라 허가 절차**입니다 — 담당자에게 허가를 받으면 남의 영역도 진행합니다.
> 다만 [규칙](#규칙)의 기술적 덫(픽스처 버전 · `CheckType` 이름 · 비밀값 · 계약 준수)은 **허가와 무관하게** 지킵니다.
> 합의로 풀 수 있는 문제가 아니라, 어기면 조용히 깨지는 것들이기 때문입니다.
...
세 사람 모두가 매일 읽는 문서·설정이라 조용히 바뀌면 다른 사람의 판단 근거가 무너집니다. 사전 합의까지 기다릴 필요는 없고, **담당자 허가를 받고 고친 뒤 무엇을 왜 바꿨는지 PR에 남기면 됩니다.** `CONTRACT.md`만은 예외로, 앱과 서버가 동시에 어긋나므로 2·3번 허가와 버전 올림이 함께 가야 합니다.
...
| 판정 **2↔3** | [`CONTRACT.md`](CONTRACT.md) | 코드가 계약을 따름. 변경은 2·3번 허가 → 버전 올림 → 코드 수정 순서(순서는 지킵니다 — 뒤집으면 배포된 서버와 앱이 어긋납니다). `CheckType` 값 **추가**는 4곳을 함께(앱 enum · `toServerType()` · 서버 `Literal` · `CHECK_TYPE_HINT`), 값 **이름 변경·삭제는 금지** — 저장된 사용자 레시피가 파싱에 실패해 조용히 사라집니다 |
| 레시피 **2↔3** | 기능명세서 §5.1.1 | **대표 레시피(쏘야)**의 완료 조건 문장·판정 유형만 §5.1.1 표가 정본이고, 나머지 레시피는 `RecipeFixtures.kt`가 정본입니다(근거는 `docs/`). `RecipeFixtures.kt`는 **3번 소유**(로직 없음, 참조처는 `CookingSessionViewModel`뿐). **문장 수정은 3번 단독**, `checkType`·단계 수·`InspectionPolicy` 변경은 2번 허가를 받고 진행 — `FakeJudgmentGatewayTest`가 단계 인덱스와 유형을 단언합니다. 편집기 UI(`MainActivity`)·저장(`AppPersistence`)·네트워크 배관은 2번 |
...
내 담당 밖을 건드려야 하면 **허가를 받고 진행**합니다. `app/` 안(1↔2)은 사후에 알리는 것으로 충분하고, `app/`↔`server/`를 넘을 때는 브랜치 + PR로 처리합니다. 권한이 애매하면 멈추지 말고 물어본 뒤 진행하고, 답을 기다릴 수 없으면 **한 일을 점검 문서에 남깁니다.**
...
`main` 반영은 PR로. **`app/`을 건드린 PR은 1번·2번이 서로 리뷰**하고, `app/`↔`server/`를 넘은 PR은 해당 담당자 리뷰를 받습니다. 리뷰가 하루 넘게 안 붙으면 무엇을 왜 바꿨는지 PR에 적고 머지한 뒤 알립니다 — 시연이 막히는 것보다 낫습니다.
