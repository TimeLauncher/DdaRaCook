...
- crop: bbox 바깥 추가 여백 없음(0%), 도마 4:3(`768×576`), 팬 1:1(`768×768`), JPEG q80

2026-08-25 폰 크롭 실사용 확인 후 22% → 10%로 축소했으나 여전히 넓다는 사용자 확인에
따라 추가 여백을 0%로 제거했다. 출력 비율을 맞추기 위한 확장만 허용하며 검출 bbox보다
작게 잘라내지는 않는다.

2026-08-26 VLM 입력 지연을 줄이기 위해 최종 출력의 가로·세로를 기존 대비 75%로
축소했다. YOLO 입력은 검출 성능 유지를 위해 기존 `640×640`을 그대로 사용한다.
...
앱은 전체 시야를 준비하고 새 사용자·추출 레시피는 `AUTO_ROI`로 도마/팬 후보 중 시선에
가장 가까운 대상을 자동 선택한다. 내장 레시피의 도마/팬 target은 선택 힌트로 유지한다.
대상 검출 실패, 모델 로드 실패, 거리 gate 탈락 시 기존 bottom-60 방식으로 안전하게 fallback한다.

기존 앱은 `cropTarget`을 보내지 않으므로 서버가 이미 정규화된 이미지를 그대로 통과시킨다.
새 앱의 수동 갤러리 판정은 `NO_CROP`을 보내며, 자동 판정만 서버 crop을 사용한다.

## 2026-08-26 `AUTO_ROI` 재검증

위 32장을 운영 ONNX로 다시 추론하고, 레시피 target을 보지 않은 채 도마·팬 전체
후보 중 시선 기준점에 가장 가까운 것을 고르는 실제 `AUTO_ROI` 조건으로 측정했다.
앱 asset과 서버 모델의 SHA-256은 동일하다.

| 지표 | 결과 |
|---|---:|
| bbox precision / recall (IoU 0.5) | 70.0% / 84.5% |
| 올바른 클래스 선택 | 27/32 (84.4%) |
| 잘못된 클래스 선택 | 3/32 (9.4%) |
| fallback | 2/32 (6.3%) |
| 활성 대상 IoU 0.5 이상 | 24/32 (75.0%) |
| 추론 중앙값 / 최대 | 69.8ms / 90.2ms |

추가 여백 0%에서는 정답 활성 객체의 95% 이상을 담은 최종 crop이 7/32(21.9%)였다.
같은 검출 결과에서 여백만 바꾸면 22%는 31/32(96.9%), 30%는 32/32(100%)였다.
출력은 어느 경우든 도마 `768×576`, 팬 `768×768`이므로 여백 변경은 VLM 입력
해상도를 늘리지 않는다.

실제 Nemotron 판정도 21쌍에 시도했으나 17쌍이 7.5초 timeout 또는 upstream 500으로
실패했다. 응답한 4쌍은 모두 정답이었지만 표본이 4개뿐이므로 VLM 정확도 수치로
사용하지 않는다. 이 결과는 crop 품질과 별도로 현재 upstream 가용성 문제를 보여준다.

이 평가는 한 주방·같은 도마와 팬의 32장이므로 회귀 검증 결과이며, 다른 주방에 대한
일반화 정확도로 표현하지 않는다.
