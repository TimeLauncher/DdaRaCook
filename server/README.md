...
**배선 완료 (T1-4·T1-5).** 앱 기본값이 실제 서버 호출입니다(`useMockJudgment = false`).
계약(`CONTRACT.md`)의 요청·응답 필드는 **하나도 바뀌지 않았습니다.**
...
| T1-5 | 통합 테스트 | ✅ 완료 — 앱이 실제 서버를 호출 (`useMockJudgment = false`) |
| T2-2 | 평가 스크립트 `eval.py` | ✅ 구현 완료 · 실데이터만 대기 |
| T2-1 | 테스트 데이터셋 촬영·라벨링 | ⏳ **현재 병목** — 안경 연동이 끝나 실기기 촬영 가능. T2-3/T2-4가 여기에 걸림 |
| B-1·B-2 | 스키마 오류 422 → 400 | ✅ 완료 (`RequestValidationError` 핸들러) |

> ✅ **배포본 확인 (2026-08-14)** — `/health` 응답:
> `promptVersion: v1` · `realJudgeReady: true` · `envMissing: []`
> ⚠️ **`debugMode: true`** — 시연 전 Render 환경변수에서 `false`로 (T3-1)
