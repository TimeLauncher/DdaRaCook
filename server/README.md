...
1. **T2-1 실기기 재촬영·라벨링** ← 지금 가장 값어치 있는 일
   대상은 쏘야 자동 판정 3단계(명세서 §5.1.1) — 1단계 썰기·3단계 투입·4단계 칼집,
   그리고 CANNOT_TELL 표본. 폰 사진으로 낸 95.2%는 제품 검증이 아니고,
   `labels.json` 23쌍은 전부 1회차 `demo-onion` 이라 지금 단계와 대응하지 않는다
2. **YOLO crop 적용본으로 판정 정확도 재측정** — 앱/서버 ROI 배선은 완료. 독립 환경 이미지 수집은 보류
3. **T2-3** 프롬프트 v1 → v2 — `STATE_CHANGE` 힌트가 1순위 (아래 ④)
4. **배포본 `DEBUG_MODE=false`** (T3-1) — 코드 기본값은 고쳤고 남은 건 Render 환경변수뿐

G-2(`CONTRACT.md` §3.2 개정)는 v1.3으로 완료됐다 — `docs/contract-compliance.md` 참조.
