...
**A-1~A-6 · B-1·B-2 · C-1은 모두 해결됐습니다 (2026-08-14 기준).** 남은 것은 G 두 건입니다.

1. **G-2** (`CONTRACT.md` §3.2 개정) — 쏘야 4단계가 *"소세지 칼집이 처음보다 벌어졌는가"*를
   비교 이미지 없이 판정하고 있어 **오류 없이 조용히 틀립니다.** 실질 결함이라 먼저
2. **G-1** (A등급 1 : B등급 3) — T2-1 재촬영 후 B등급 정확도를 실제로 재고 결정
...
### [ ] G-1. A등급 1개 : B등급 3개 — "A등급 중심"이 성립하지 않음

기능명세서 §5.1(226줄)과 §7(343줄)은 **"A등급 중심 + B등급 단계 1개만"** 입니다. 새 구성은 이렇습니다.
...
### [ ] G-2. `CONTRACT.md` §3.2 개정 필요

A-1 참조. 전송 정책 기준을 판정 유형에서 **"완료 조건이 시작 시점 대비 변화를 묻는가"** 로 바꿔야 합니다. 계약서는 3인 합의 문서라 이 점검 문서에만 기록하고 원문은 손대지 않았습니다.
