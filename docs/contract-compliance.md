...
**왜 중요한가** — 이미지 2장이면 페이로드가 2배가 되어 왕복 3초 목표(기능명세서 12절 #4)를 압박합니다. 실측은 1장 2549ms / 2장 3648ms, 타임아웃 1/23 → 5/23 (`server/notes/nemotron.md` §9).

> 🔄 **2026-08-13 갱신 — 이 항목은 "앱 위반"에서 "계약 결함"으로 성격이 바뀌었습니다.**
>
> 대표 레시피가 소세지야채볶음으로 교체되면서 `STATE_CHANGE` 단계가 둘이 됐는데 **전송 정책이 서로 반대**입니다 (기능명세서 §5.1.1).
>
> | 단계 | 완료 조건 | 판정 성격 | `startImage` |
> |---|---|---|---|
> | 1 야채 썰기 | 도마 위에 통째로 남은 야채 덩어리가 없는가 | 절대 | null |
> | 4 소세지 | 소세지 칼집이 **처음보다** 벌어졌는가 | 상대 | **전송** |
>
> §3.2가 **판정 유형 단위**로 정책을 정한 것이 틀렸습니다. 기준은 **"완료 조건이 시작 시점 대비 변화를 묻는가"** 여야 합니다.
>
> **그래서 앱 조건을 지금 고치지 마십시오.** `checkType`만으로는 두 단계를 갈라낼 수 없습니다. 계약 §3.2 개정(3인 합의)이 먼저이고, 그 뒤 전송 여부를 레시피 단계에 직접 두는 방식이 됩니다. T2-4(1장 vs 2장) 결과와 함께 올릴 항목입니다.

**고치는 법** — 위 개정 전까지 **보류**. `SAMPLE_JPEG_BASE64` 더미를 실제 캡처로 바꾸는 것(T1-5 배선)이 먼저입니다.
...
> 🚨 **2026-08-13 — 그 검사도 이걸 못 잡고 있었습니다.** `.env`의 **현재** 값만 찾기 때문에, 2026-08-12에 교체한 **옛 토큰**은 `JudgeApiService.kt:12`에 그대로 남은 채 `✅ 안전`이 떴습니다. 토큰 **모양**(`cookassist-` + 20자 이상)을 찾는 검사를 추가했고, 지금은 이렇게 나옵니다.
>
> ```
> 🚨 TEAM_TOKEN 모양의 값이 추적 중인 파일에 있습니다 — 폐기된 값이라도 제거하세요.
>    ../app/src/main/java/com/example/myapplication/JudgeApiService.kt:12:...
> ```
>
> **⚠️ 이 줄이 남아 있는 동안 `check_secrets.sh`는 exit 1이라 push 전 검사가 계속 실패합니다.** 폐기된 값이라 보안 위험은 없지만, 다음 사람이 그 자리에 새 토큰을 넣는 걸 막는 게 목적입니다. 2번이 A-6을 처리하면 해제됩니다.
...
**A-1~A-6 · B-1·B-2 · C-1 · G-2 가 해결됐습니다 (2026-08-14 기준).** 남은 것은 G-1 한 건입니다.

1. **G-1** (A등급 0 : B등급 3) — T2-1 재촬영 후 B등급 정확도를 실제로 재고 결정.
   5단계가 시간 전용이 되면서 A등급이 0개라 **판정 성능이 곧 제품 성능**입니다
2. (신규) **최초 검사 시점이 런타임에서 30초로 덮이는 문제** — 명세서 §5.1.1이 정한
   3·4단계 2분 대기가 `withAutomaticInspectionInterval()` 때문에 지켜지지 않습니다
...
## G. 명세서 미합의 — 2026-08-13 레시피 교체로 생긴 것

대표 레시피가 **김치볶음밥 → 소세지야채볶음(쏘야)** 으로 교체됐습니다 (기능명세서 §5.1.1). 교체 자체는 합의됐고, 아래 두 건이 남았습니다.

### [ ] G-1. B등급 정확도가 측정된 적이 없음

현재 구성입니다 (정본 `RecipeFixtures.kt` · 명세서 §5.1.1은 이 표를 따라 갱신됨).

| # | 단계 | 유형 | 등급 | 시작 이미지 |
|---|---|---|---|---|
| 1 | 야채 자르기 | STATE_CHANGE | B | 없음 (절대 판정) |
| 2 | 양념장 만들기 | TIME_ONLY | — | — |
| 3 | 팬에 야채·소세지 투입 | PRESENCE | **A** | 없음 |
| 4 | 중약불로 볶기 — 소세지 칼집 벌어짐 | STATE_CHANGE | B | **전송** (상대 판정) |
| 5 | 양념장 넣고 3분 | TIME_ONLY | — | — |

> 2026-08-14에 5단계가 시간 전용이 되면서 A등급이 0개가 됐다가, 재료 투입을 존재 판정
> 단계로 세우면서 3단계가 A등급으로 들어왔습니다. 지금은 **자동 판정 3개 중 A 1 · B 2** 입니다.
> 같은 변경으로 **색상 변화(양파 반투명) 단계가 사라졌습니다** — 볶는 정도를 보는 판정이
> 레시피에 하나도 없습니다.

**규칙 위반보다 무서운 것** — 우리는 **B등급 정확도를 측정한 적이 없습니다.** 1회차 세트의 B등급 14쌍 중 13쌍이 NOT_DONE이라 "항상 NOT_DONE"으로도 만점이 나왔고, 그래서 보고한 숫자는 A등급 85.7%뿐입니다. 더구나 `testdata/labels.json` 23쌍은 전부 `demo-onion` 이고 그중 10쌍이 **지금은 존재하지 않는 COLOR_CHANGE** 라, 현재 레시피의 어느 단계와도 대응하지 않습니다.

**처리 순서** — 2회차 촬영 → `eval.py`로 B등급 정확도 측정 → 그 숫자로 결정합니다.
촬영 대상은 위 표의 1·3·4단계와 CANNOT_TELL 표본(김·가림·프레임 이탈·흔들림)입니다.

- 목표치를 넘기면 → "**자동 진행을 맡기는 조리 중** B등급 단계는 1개"로 정밀화. 1단계(썰기)는 준비 구간이라 오탐이 나도 손이 깨끗할 때 "이전"(F5-4) 한마디로 복구됩니다. 4단계 볶기 오탐과 피해가 같지 않습니다
- 미달이면 → 4단계를 존재 판정("팬 안에 비엔나소세지가 들어 있는가", IDENTIFY·A)으로 되돌립니다. 같은 사진에 질문만 바꿔 `pairId`를 하나 더 만들면 추가 촬영 없이 그 라벨을 확보할 수 있습니다 (1회차 `onion_id_*` 가 쓴 방식)

**연속 DONE 기준도 함께 봅니다.** 명세서 설계는 "A등급 1회 / B등급 2회"였는데 픽스처는 전 단계 `requiredConsecutiveDone = 1` 입니다. B등급 단계에서 오탐 한 번이 곧 자동 진행입니다.

### [x] G-2. `CONTRACT.md` §3.2 개정 — **완료 (v1.3 · 2026-08-14)**
...
- 기준 사진 촬영을 단계 시작 **15초 뒤**로 지연 (`BASELINE_CAPTURE_DELAY_SECONDS`).
  직후에 찍으면 빈 팬이 기준이 되어 비교가 성립하지 않습니다
- 쏘야 4단계가 이제 실제로 2장 판정을 받습니다

> 남은 것: **정확도 영향 미측정.** 4단계가 1장→2장이 되면서 판정이 어떻게 달라지는지는
> 2회차 촬영(T2-1) 후 `eval.py --mode both` 로 재야 합니다.
