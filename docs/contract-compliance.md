...
### [ ] G-1. A등급 0개 : B등급 3개 — "A등급 중심"이 성립하지 않음

기능명세서 §5.1(226줄)과 §7(343줄)은 **"A등급 중심 + B등급 단계 1개만"** 입니다. 현재 구성은 이렇습니다.

| # | 단계 | 유형 | 등급 |
|---|---|---|---|
| 1 | 야채 자르기 | STATE_CHANGE | B |
| 3 | 야채 볶기 | COLOR_CHANGE | B |
| 4 | 소세지 칼집 벌어짐 | STATE_CHANGE | B |
| 5 | 양념장 투입 | ~~PRESENCE~~ → **TIME_ONLY** | — (판정 없음) |

> 2026-08-14에 5단계가 시간 전용으로 바뀌면서 **A등급이 유일하게 하나 있던 것마저 사라졌습니다.**
> 서버가 판정하는 단계 3개가 전부 B등급입니다. 아래 서술의 "A등급 1개"는 이제 0개로 읽으십시오.

**규칙 위반보다 무서운 것** — 우리는 **B등급 정확도를 측정한 적이 없습니다.** 1회차 세트의 B등급 14쌍 중 13쌍이 NOT_DONE이라 "항상 NOT_DONE"으로도 만점이 나왔고, 그래서 보고한 숫자는 A등급 85.7%뿐입니다. 판정 4개 중 3개가 B인 레시피를 **성능을 모르는 채로** 시연하려는 상태입니다.

**처리 순서** — 2회차 촬영 → `eval.py`로 B등급 정확도 측정 → 그 숫자로 결정합니다.

- 목표치를 넘기면 → §5.1·§7을 "**자동 진행을 맡기는 조리 중** B등급 단계는 1개"로 정밀화. 1단계(썰기)는 준비 구간이라 오탐이 나도 손이 깨끗할 때 "이전"(F5-4) 한마디로 복구됩니다. 3단계 볶기 오탐과 피해가 같지 않습니다
- 미달이면 → 4단계를 존재 판정("팬 안에 비엔나소세지가 들어 있는가", IDENTIFY·A)으로 되돌립니다. **그때 쓸 라벨은 추가 촬영 없이 확보됩니다** (`server/testdata/raw/촬영목록_2회차.md` 참조)

**이 결정 전까지 §5.1·§7·F1-5의 "A등급 중심" 문장은 고치지 않습니다.**
